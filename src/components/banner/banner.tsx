...
function Banner ({banner}: BannerProps): JSX.Element {
  ...
  return (
    <div className="banner">
      ...
      <p className="banner__info"><span className="banner__message">Новинка!</span><span className="title title--h1">{banner?.name}</span><span className="banner__text">Профессиональная камера от&nbsp;известного производителя</span><Link className="btn" to={detailsLink}>Подробнее</Link></p>
    </div>
  );
}
